...
const jackpotConfig = {
  bronze: { reset: 200, color: '#CD7F32' },
  argent: { reset: 750, color: '#C0C0C0' },
  or: { reset: 2500, color: '#FFD700' },
  diamant: { reset: 10000, color: '#7dd3fc' },
};
const jackpotSplits = { bronze: 0.35, argent: 0.25, or: 0.20, diamant: 0.20 };
...
function contributeJackpots(amount) {
  ...
  for (const key of Object.keys(jackpots)) {
    ...
    jackpots[key].relay += net * jackpotSplits[key] * 0.12;
...
app.get('/audio-health', (req, res) => {
  ...
  const files = Object.fromEntries(expectedFiles.map((name) => [name, fs.existsSync(path.join(EXTERNAL_AUDIO_DIR, name))]));
...
const jackpotTierRank = { bronze: 1, argent: 2, or: 3, diamant: 4 };

function jackpotTierFromRawOdds(rawOdds) {
  if (!rawOdds) return null;
  if (rawOdds > 7000) return 'diamant';
  if (rawOdds >= 2501) return 'or';
  if (rawOdds >= 1001) return 'argent';
  if (rawOdds >= 200) return 'bronze';
  return null;
}
