...
function closeHandDetailsSoon(wrap, delay = 180) {
 if (!wrap) return;
 if (wrap._closeDetailsTimer) clearTimeout(wrap._closeDetailsTimer);
 wrap._closeDetailsTimer = setTimeout(() => {
 const cardsNode = wrap.querySelector(".hand-cards");
 const detailsNode = wrap.querySelector(".hand-details");
 const jackpotNode = wrap.querySelector(".hand-odds.jackpot-call");
 const hoveringCards = !!cardsNode && cardsNode.matches(":hover");
 const hoveringDetails = !!detailsNode && detailsNode.matches(":hover");
 const hoveringJackpot = !!jackpotNode && jackpotNode.matches(":hover");
 if (hoveringCards || hoveringDetails || hoveringJackpot) return;
...
function buildHandsUI() {
 ...
 pinHandDetailsBriefly(wrap, 320);
 onHandClick(i, ph);
 });
 ...
 const jackpotNode = wrap.querySelector(".hand-odds.jackpot-call");
 if (to && (details.contains(to) || (jackpotNode && jackpotNode.contains(to)))) return;
 closeHandDetailsSoon(wrap);
 });

 details.addEventListener("mouseenter", () => {
 if (wrap.classList.contains("details-open")) openHandDetails(wrap);
 });

 details.addEventListener("mouseleave", (e) => {
 const to = e.relatedTarget;
 const jackpotNode = wrap.querySelector(".hand-odds.jackpot-call");
 if (to && (cards.contains(to) || (jackpotNode && jackpotNode.contains(to)))) return;
 closeHandDetailsSoon(wrap);
 });

 const handleJackpotOddsEnter = () => {
 if (odds.classList.contains("jackpot-call")) {
 openHandDetails(wrap);
 }
 };

 const handleJackpotOddsLeave = (e) => {
 if (!odds.classList.contains("jackpot-call")) return;
 const to = e.relatedTarget;
 if (to && (cards.contains(to) || details.contains(to))) return;
 closeHandDetailsSoon(wrap);
 };

 odds.addEventListener("mouseenter", handleJackpotOddsEnter);
 odds.addEventListener("pointerenter", handleJackpotOddsEnter);

 odds.addEventListener("mouseleave", handleJackpotOddsLeave);
 odds.addEventListener("pointerleave", handleJackpotOddsLeave);

 detailsPanel.addEventListener("pointerdown", () => {
 pinHandDetailsBriefly(wrap, 320);
 });
...
function renderHands() {
 ...
 if (jackpotLabel) {
 odds.textContent = jackpotLabel;
 odds.classList.add("jackpot-call");
 const type = currentJackpotType;
 const jackpotLocked = !!type && hasJackpotTypeAlreadyBetOnTarget(type, "hand", i);
...
if (tieOddsEl) {
 ...
 if (jackpotLabel) {
 tieOddsEl.textContent = jackpotLabel;
 tieOddsEl.classList.add("jackpot-call");
 const type = currentTieJackpotType;
 const jackpotLocked = !!type && hasJackpotTypeAlreadyBetOnTarget(type, "tie", -1);
...
function launchChipFlight(targetNode) {
 ...
 const sourceNode =
  document.querySelector(".stat #bankroll")?.closest(".stat") ||
  document.getElementById("bankroll")?.closest(".stat") ||
  document.getElementById("betPanel");
 ...
 chip.style.setProperty("--chip-mid-x", `${(target.x - source.x) * 0.45}px`);
 chip.style.setProperty("--chip-mid-y", `${Math.min(-10, (target.y - source.y) * 0.18)}px`);
...
function launchWinChipFlight(sourceNode, amount) {
 ...
 const targetNode = document.querySelector(".stat.total-wins") || document.getElementById("totalWins");
 ...
 const flashNode = document.querySelector(".stat.total-wins");
 if (flashNode) {
...
function launchWinCoinBurst(sourceNode, amount) {
 ...
 const endJitterX = ((i % 4) - 1.5) * (amount >= 600 ? 14 : 10);
 const endJitterY = ((i % 3) - 1) * (amount >= 600 ? 9 : 6);
