...
function jackpotTypeForOddsValue(oddsValue) {
 ...
 if (oddsValue > 7000) return "diamant";
 if (oddsValue >= 2501) return "or";
 if (oddsValue >= 1001) return "argent";
 if (oddsValue >= 200) return "bronze";
 return null;
}
...
function getJackpotAvailability() {
 return {
  bronze: !!getJackpotTarget("bronze"),
...
function shouldSuppressJackpotOfferAtPhase(type, targetKind, targetIndex, ph) {
 return ph === "flop" && hasJackpotTypeBetOnTargetAtPhase(type, targetKind, targetIndex, "pre");
}
...
function updateJackpotDisplays() {
 ...
 for (const type of JACKPOT_TYPES) {
  ...
  if (boxEl) {
   ...
   boxEl.classList.toggle('flash-bronze', hasLiveBet && type === 'bronze');
...
function applyServerJackpotPayouts(jackpotPayouts = []) {
 ...
 jackpotBets = { bronze: [], argent: [], or: [], diamant: [] };
 jackpotRoundStake = { bronze: 0, argent: 0, or: 0, diamant: 0 };
 updateJackpotDisplays();
}
