...
function updateToggleButton(){var btn=document.getElementById('btnToggleTutorial');if(!btn)return;var ok=false,sp=document.getElementById('splashScreen'),su=document.getElementById('roundSetupOverlay'),ab=document.getElementById('btnAbandon');if(tutoActive)ok=true;if(sp&&!sp.classList.contains('hidden'))ok=true;if(su&&!su.classList.contains('hidden'))ok=true;if(typeof roundFinished!=='undefined'&&roundFinished)ok=true;if(ab&&ab.style.display!=='none'&&ab.offsetParent!==null)ok=true;btn.disabled=!ok;btn.style.opacity=ok?'1':'0.38';btn.style.pointerEvents=ok?'auto':'none';}
